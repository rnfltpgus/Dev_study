...
## 8. NPX, Degit
parcel, webpack bundler를 통해 만든 템플릿을 터미널을 통해 손쉡게 다운로드 하는 방법
원시적인 방법으로는 `github 저장소`에 들어가 `code` 라는 버튼을 눌러 알집을 받아 원하는 폴더에 넣어 손쉽게 시작하는 방법도 있다.

<br>

### 설치방법 및 설정
1. 터미널을 열어 `맥은 ls, 윈도우는 dir, cd, ..cd`등을 입력하여 다운로드 받을 로컬PC 내 폴더 위치를 지정해 준다. (설명이 이상하지만 이정도는 할수 있을 것이라고 생각함)

2. 지정을 하였으면 아래의 코드를 작성한다. (명령어로 '()'는 포함되지않음)
```
npx degit (git저장소명) (다운받을 폴더명)
```
`degit` 명령어를 통해 명시한 github 저장소의 코드를 지정한 경로의 폴더로 다운로드 받을 수 있다. 이 명령어를 동작시키기 위해서는 `degit`을 실제로 설치해야되지만, 설치하지 않고도 동작시킬 수 있게 `npx`를 앞에 입력해 준다. `npx`는 `node.js` 환경에서 사용할 수 있음

3. 위의 명령어를 입력 후 곧 설치가 완료되었다는 문구를 터미널을 통해 확인할 수 있다.
```
npx: 1개의 패키지를 @초만에 설치했습니다.
어쩌구 저쩌구 ~~~
```
...
## 추가설명
`NPX, Degit을 이용하여 탬플릿을 다운받아 시작한는 것이 좋은 것`은 버전관리가 전혀 되지않은 상태라서 처음부터 버전관리를 진행할 수 있다는 것이 좋은 장점이고 `새로운 프로젝트를 시작할때 적합`하다고 볼수있다.

`git init으로 해서 git clone 으로 받아오는 방식`은 원격저장소 내에 버전의 내역까지 가져오기 때문
